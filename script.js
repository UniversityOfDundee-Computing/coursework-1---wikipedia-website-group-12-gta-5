function TSwitch(){
    document.getElementById("child2").style.display = "none"
    document.getElementById("child3").style.display = "none"
    document.getElementById("child1").style.display = "none"
    document.getElementById("TrevorImgOpenTab").style.display = "block"
    document.getElementById("TrevorTBox1").style.display = "block"
    document.getElementById("TrevorTBox2").style.display = "block"
    document.getElementById("TrevorTBox3").style.display = "block"
    document.getElementById("child1").style.marginTop = "0px"
    document.getElementById("child1").style.justifyContent = "space-evenly"
}
function TCloseTab(){
    document.getElementById("child2").style.display = "block"
    document.getElementById("child3").style.display = "block"
    document.getElementById("child1").style.display = "block"
    document.getElementById("TrevorImgOpenTab").style.display = "none "
    document.getElementById("TrevorTBox1").style.display = "none"
    document.getElementById("TrevorTBox2").style.display = "none"
    document.getElementById("TrevorTBox3").style.display = "none"
}

function MSwitch(){
    document.getElementById("child2").style.display = "none"
    document.getElementById("child3").style.display = "none"
    document.getElementById("child1").style.display = "none"
    document.getElementById("MichaelImgOpenTab").style.display = "block"
    document.getElementById("MichaelTBox1").style.display = "block"
    document.getElementById("MichaelTBox2").style.display = "block"
    document.getElementById("MichaelTBox3").style.display = "block"
    document.getElementById("child2").style.marginTop = "0px"
    document.getElementById("child1").style.justifyContent = "space-evenly"
}
function MCloseTab(){
    document.getElementById("child2").style.display = "block"
    document.getElementById("child3").style.display = "block"
    document.getElementById("child1").style.display = "block"
    document.getElementById("MichaelImgOpenTab").style.display = "none "
    document.getElementById("MichaelTBox1").style.display = "none"
    document.getElementById("MichaelTBox2").style.display = "none"
    document.getElementById("MichaelTBox3").style.display = "none"
}
function FSwitch(){
    document.getElementById("child2").style.display = "none"
    document.getElementById("child3").style.display = "none"
    document.getElementById("child1").style.display = "none"
    document.getElementById("FranklinImgOpenTab").style.display = "block"
    document.getElementById("FranklinTBox1").style.display = "block"
    document.getElementById("FranklinTBox2").style.display = "block"
    document.getElementById("FranklinTBox3").style.display = "block"
    document.getElementById("child2").style.marginTop = "0px"
    document.getElementById("child1").style.justifyContent = "space-evenly"
}
function FCloseTab(){
    document.getElementById("child2").style.display = "block"
    document.getElementById("child3").style.display = "block"
    document.getElementById("child1").style.display = "block"
    document.getElementById("FranklinImgOpenTab").style.display = "none "
    document.getElementById("FranklinTBox1").style.display = "none"
    document.getElementById("FranklinTBox2").style.display = "none"
    document.getElementById("FranklinTBox3").style.display = "none"
}
document.getElementById("PlotButton").onclick = function () {
    location.href = "Plot.html";
};